...
def describe_choices_help(sides: int, n: int, ss: set[int]) -> str:
    ...
    for a, b in zip(sorted(start), sorted(stop)):
        if a == b:
            ...
            descs.append("%d dice making between %d and %d" % (n, a + n, b + n))
...
def describe_strategy(
    dice_count: int, sides: int, values: Sequence[Sequence[int | fractions.Fraction]]
) -> None:
    ...
    for n, row in enumerate(values[:-1]):
        print(
            "   " * (dice_count - n) + " ".join("%02d" % v_sort.index(v) for v in row)
        )
    seen = set()
    actions = []
    for v in reversed(v_sort):
        ...
        for n, s in all_dice:
            ...
            all_seen_before = all(
                any((len(ss), sum(ss)) in seen for ss in subsequences(outcome))
    ...
    for i, dice in enumerate(actions):
        ...
        for n, s in dice:
            ...
            if any(
                can_cooccur(sides, dice_count, n, s, n_, s_)
                for j in range(i)
                for n_, s_ in actions[j]
            ):
                a.append((n, s))
            else:
    ...
    for i in range(len(conditional_actions)):
        ...
        for j in range(i, 0, -1):
            ...
            conditional_actions[j - 1], conditional_actions[j] = (
                conditional_actions[j],
                conditional_actions[j - 1],
            )
    ...
    for i, dice in enumerate(actions):
        p_win = min(
            max(below_max_prob[dice_count - n][s], above_max_prob[dice_count - n][s])
            for n, s in dice
        )
        n_outcomes = 0
        for n, s in dice:
        ...
        print(
            "keep %s (%.2f%%, %s)"
            % (describe_choices(sides, dice), float(100 * p_win), n_outcomes)
        )
