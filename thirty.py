...
def play_game(dice_count: int, sides: int, strategy: Strategy) -> int:
    ...
    while outcome:
        ...
        if reroll:
            print(
                "Sum: %2d  You chose to reroll: %s"
                % (s + dice_count - len(reroll), [a + 1 for a in reroll])
            )
...
def main() -> None:
    ...
    if args.lucky:
        ...
        print(
            "Probability of low success: {:.2%}".format(
        ...
        print("... in first roll: {} = {:.2%}".format(below_1st, float(below_1st)))
        for multiplicity, outcome in below_1st_rolls:
            print("%3d * %s" % (multiplicity, "".join(str(v + 1) for v in outcome)))
        above_prob = optimal_values(dice_count, sides, is_above)
        print(
            "Probability of high success: {:.2%}".format(
        ...
        print("... in first roll: {} = {:.2%}".format(above_1st, float(above_1st)))
        for multiplicity, outcome in above_1st_rolls:
            print("%3d * %s" % (multiplicity, "".join(str(v + 1) for v in outcome)))
        return

    if args.minimax:
        print("Compute minimax strategy for %d %d-sided dice..." % (dice_count, sides))
        ...
        print(
            "Compute self-defense strategy for %d %d-sided dice..."
            % (dice_count, sides)
        )
        max_lose = 14

        def score(s: int) -> int:
            return max(-max_lose, s - 24) if 5 <= s < 24 else 0
    ...
    if args.describe:
        ...
        print(
            "Probability of winning: {:.2%}".format(
            ...
            if min_sum == max_sum:
                ...
                for reroll, ss in itertools.groupby(rerolls):
                    ...
                    if i == min_sum and j == max_sum + 1:
                        ...
                        print(
                            "If you have between %s and %s, I would %s"
                            % (i, j - 1, reroll)
                        )
                    i = j
